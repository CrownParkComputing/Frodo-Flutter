...
constexpr int AUTOSTART_FALLBACK_FRAMES = 18;
...
void C64::InjectKey(SDL_Scancode key, bool key_up)
{
	if (TheDisplay && TheCIA1) {
		TheDisplay->InjectKey(key, key_up, TheCIA1->KeyMatrix, TheCIA1->RevMatrix, &joykey);
	}
}
...
void C64::NewPrefs(const Prefs *prefs)
{
	...
	if (prefs->AutoStart) {
		pending_autostart_frames = AUTOSTART_FALLBACK_FRAMES;
...
void C64::schedule_autostart_fallback()
{
	pending_autostart_frames = AUTOSTART_FALLBACK_FRAMES;
}
...
void C64::vblank()
{
	if (pending_autostart_frames > 0) {
		--pending_autostart_frames;
		if (pending_autostart_frames == 0 && ThePrefs.AutoStart) {
			AutoStartOp();
		}
	}
			...
			if (!TheCPU1541->Idle) {
				...
				while (cycles >= 0 || cycles_1541 >= 0) {
					if (cycles > cycles_1541) {
						int used = TheCPU->EmulateLine(1);
						TheTape->EmulateLine(used);
						cycles -= used;
					} else {
...
uint8_t C64::poll_joystick(int port)
{
	...
	// Always apply software override mask (active-low) so Android/controller
	// events received outside SDL joystick APIs still drive CIA joystick bits.
	j &= joy_override[port];
...
/*
 *  Android software joystick override (used when no SDL game controller is open)
 */

void C64::SetJoystickOverride(int port, uint8_t bits)
{
	if (port >= 0 && port < 2) {
		joy_override[port] = bits;
	}
}
...
void C64::SetTapeSpeedMultiplier(int multiplier)
{
	TheTape->SetSpeedMultiplier(multiplier);
}
...
void C64::AutoStartOp()
{
	...
	if (! ThePrefs.LoadProgram.empty() ) {
		...
		// Starting from drive 8. Archive media such as .t64 behave more like
		// "load first entry" than a disk-side autoboot, so prefer LOAD"" there.
		int type = 0;
		bool is_archive = IsMountableFile(ThePrefs.DrivePath[0], type) && type == FILE_ARCH;
		if (is_archive) {
			write_to_screen("load\"\",8");
		} else {
		...
		// Execute LOAD command directly from keyboard buffer (no RUN).
		set_keyboard_buffer("LOAD\"\"\x0d");

		// Rewind and press PLAY on tape
		RewindTape();
		SetTapeButtons(TapeState::Play);
		ShowNotification("LOAD\"\" - Press PLAY on tape");
	}
}

void C64::QueueDiskAutoLoad()
{
	...
	set_keyboard_buffer("\x0dRUN\x0d");
	ShowNotification("LOAD\"*\",8,1 then RUN");
}

void C64::QueueTapeAutoLoad()
{
	if (ThePrefs.TapePath.empty()) {
		return;
	}
