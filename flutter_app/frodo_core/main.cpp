...
int Frodo::ReadyToRun()
{
	...
	if (prefs_path.empty()) {
	#ifdef ANDROID
		// In the Flutter bridge build, emulation can run on a native worker thread.
		// Avoid SDL_GetPrefPath here because it can touch JNI from a non-Java thread.
		prefs_path = fs::path("config");
		snapshot_path = fs::path("snapshots");
	#else
		...
		if (path != nullptr && path[0] != '\0') {
			...
			// Fallback when SDL Android JNI path helpers are not fully ready yet.
			prefs_path = fs::path("config");
			snapshot_path = fs::path("snapshots");
		}
	#endif
		...
		if (! fs::exists(snapshot_path)) {
			std::error_code ec;
			fs::create_directories(snapshot_path, ec);
		}
