...
void Tape::SetSpeedMultiplier(int multiplier)
{
	if (multiplier <= 1) {
		speed_multiplier = 1;
...
void Tape::EmulateLine(int cycles)
{
	if (read_pulse_length <= 0 || cycles <= 0) {
		return;
	}

	int step = speed_multiplier;
	if (step < 1) {
		step = 1;
	}
	int remaining = cycles * step;
	while (remaining > 0 && read_pulse_length > 0) {
		if (read_pulse_length > remaining) {
			read_pulse_length -= remaining;
			return;
		}
		remaining -= read_pulse_length;
		read_pulse_length = 0;
		trigger_read_pulse();
		if (read_pulse_length <= 0) {
			return;
