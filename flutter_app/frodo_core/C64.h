...
public:
	...
	void AutoStartOp();
	void QueueDiskAutoLoad();
	void QueueTapeAutoLoad();
	...
	void InjectKey(SDL_Scancode key, bool key_up);

	void SetTapeButtons(TapeState pressed);
	void SetTapeControllerButton(bool pressed);
	void SetTapeSpeedMultiplier(int multiplier);

	void SetJoystickOverride(int port, uint8_t bits);  // Android software joystick
	void RewindTape();
	void ForwardTape();
...
private:
	...
	void schedule_autostart_fallback();
	...
	uint8_t joy_override[2] = { 0xff, 0xff };	// Android software joystick override
	...
	int pending_autostart_frames = 0;		// Fallback auto-start countdown after reset
