...
/*
 *  Copy VIC bitmap to ARGB buffer
 */

void Display::CopyARGB(uint32_t * out) const
{
	const uint8_t * inPixel = vic_pixels;
	for (unsigned y = 0; y < DISPLAY_Y; ++y) {
		for (unsigned x = 0; x < DISPLAY_X; ++x) {
			out[x] = 0xff000000 | palette[inPixel[x]];
		}
		inPixel += DISPLAY_X;
		out += DISPLAY_X;
	}
...
void Display::InjectKey(SDL_Scancode key, bool key_up, uint8_t *key_matrix, uint8_t *rev_matrix, uint8_t *joystick)
{
	translate_key(key, key_up, key_matrix, rev_matrix, joystick);
}
void Display::PollKeyboard(uint8_t *key_matrix, uint8_t *rev_matrix, uint8_t *joystick)
{
	...
	while (SDL_PollEvent(&event)) {
		switch (event.type) {
			// Map controller buttons to keyboard/tape actions
			...
			case SDL_CONTROLLERBUTTONUP: {
				...
				if (button_mapping.count(button) > 0) {
					...
					if (keycode < 64) {
						...
						the_c64->SetTapeControllerButton(event.type != SDL_CONTROLLERBUTTONUP);
					}
				}
				break;
			}
